...
$sql = "SELECT c.*, 
        COUNT(DISTINCT cc.id_commande) as total_commandes,
        SUM(cc.total) as montant_total 
        FROM client c
        LEFT JOIN commandeclient cc ON c.id_client = cc.id_client
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'ajouter') {
    ...
    $sql = "INSERT INTO client (nom, email, telephone, adresse) 
    ...
    header("Location: gestion_clients.php?depot=$depot_id&success=1");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'modifier') {
    ...
    header("Location: gestion_clients.php?depot=$depot_id&success=2");
    exit();
}
?>
...
<body>
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <?php if (isset($_GET['success'])): ?>
                    <div class="alert alert-success alert-dismissible fade show">
                        <?php 
                        if ($_GET['success'] == 1) echo "Client ajouté avec succès.";
                        if ($_GET['success'] == 2) echo "Client modifié avec succès.";
                        ?>
                            ...
                            <tbody>
                                <?php foreach ($clients as $client): ?>
                                    <tr>
                                        ...
                                        <td><?php echo $client['total_commandes']; ?></td>
                                        <td><?php echo number_format($client['montant_total'] ?? 0, 0, ',', ' '); ?> FCFA</td>
                                        <td>
                                            <button class="btn btn-sm btn-primary" onclick="editClient(<?php echo $client['id_client']; ?>)">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button class="btn btn-sm btn-info" onclick="viewClientDetails(<?php echo $client['id_client']; ?>)">
    ...
    <script>
        ...
        function editClient(clientId) {
            $.get('get_client.php', {id: clientId}, function(client) {
                $('#edit_id_client').val(client.id_client);
        ...
        function viewClientDetails(clientId) {
            $.get('get_client_details.php', {id: clientId}, function(details) {
                let content = `
                ...
                details.commandes.forEach(commande => {
                    content += `
                        <tr>
                            ...
                            <td><span class="badge bg-${commande.statut === 'Livrée' ? 'success' : 'warning'}">${commande.statut}</span></td>
                        </tr>`;
                });
